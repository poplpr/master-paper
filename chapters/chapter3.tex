...
文本来源追踪的任务可以数学化地表述为一个文本分类问题。我们定义一个文本语料库 \( T = \{t_1, t_2, \ldots, t_n\} \)，其中每个文本 \( t_i \) 被表示为一个字符串，表示一段文本内容。我们的目标是将每个文本 \( t_i \) 分类到一个预定义的类别集合 \( C = \{c_1, c_2, \ldots, c_k\} \)。其中，为方便理解，不妨将类别集合 \( C \) 与数据集中的类别一一对应，即：\( C = \{c_1, c_2, \ldots, c_k\} = \{ \text{Human}, \text{GPT3.5}, \text{GPT4o}, \text{Gemini}, \text{Qwen}, \text{DeepSeek}\}\)。
...
2. \textbf{分类过程}：对于新的文本 \( t_{new} \)，特征提取过程生成 \( x_{new} \)。随后，利用训练好的模型进行预测：
   ...
   其中，\( \hat{y} \) 表示模型预测的类别。在本任务中，其取值为多元分类结果：人类书写文本或某种大语言模型生成文本。这个步骤的核心在于模型能够有效地将新文本映射到其可能的类别中，展现出其在特征空间中的判断能力。
