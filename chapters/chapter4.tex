...
准确率的优势在于其计算方法简单且易于理解，适用于类别分布均衡的情况。当正负样本比例接近时（例如50\%的患病个体和50\%的健康个体），这一指标能够有效地评估模型的性能。然而，其局限性在于对不平衡数据的敏感性——例如，当99\%的样本为负类时，模型若仅预测负类便可获得99\%的准确率，但这将完全忽略正类的识别。因此，在如欺诈检测或罕见疾病诊断等少数类关键场景中，需结合精确率、召回率等其他指标进行综合评估。

在实际应用中，准确率常用于多分类任务（如手写数字识别）以及快速模型验证阶段。然而，高准确率并不一定意味着模型的优越性。例如，在某些特定数据分布下，选择题全选某一选项可能导致虚高的准确率，此时需要结合混淆矩阵进行深入分析。随着不平衡学习技术的发展，未来准确率将更多地作为基础指标，与其他评估方法协同使用，以提供更全面的模型性能评估。
...
精准率的应用场景具有显著特征。当误报（False Positive）的代价较高时，例如在垃圾邮件过滤或金融风险控制领域，精准率便成为主要考量指标。在这些情况下，将正常邮件误判为垃圾邮件（FP）可能导致重要信息的丢失，而将正常交易误判为欺诈（FP）则可能引发客户投诉，因此需要尽量降低FP值以提高精准率。实验结果表明，卷积神经网络（CNN）等深度学习模型通过调整分类阈值，可以在图像分类任务中将精准率提升至83\%以上。

然而，精准率的使用也存在明显的局限性。单独依赖精准率无法全面反映模型对正类样本的覆盖能力，可能掩盖高漏报（False Negative）的问题。例如，在癌症筛查中，过于追求高精准率可能导致实际患者未被检出（FN），因此需要结合召回率（Recall）来计算 F1 分数，以进行综合评估。研究还表明，当样本分布严重失衡时（例如正类仅占0.4\%），单纯依赖精准率可能导致误导性结论，此时应结合ROC曲线等工具进行分析。
...
召回率的应用场景具有明显的特征。当漏检（False Negative）的代价较高时，例如在金融欺诈检测或安全监控领域，该指标便成为优化的首要目标。实验结果表明，通过调整分类阈值或采用集成学习方法，深度学习模型在图像识别任务中能够将召回率提升至85\%以上。然而，召回率与精确率（Precision）之间存在权衡关系——过度追求高召回率可能导致误报的增加，例如正常交易被误判为欺诈的比例上升。因此，在实际应用中，通常将召回率与 F1 分数结合，以进行综合评估。

尽管召回率是一个重要的指标，但其也存在明显的局限性。在样本分布严重失衡的情况下（例如正类仅占1\%），单独依赖召回率可能得出误导性结论。例如，在垃圾邮件分类器中，如果将所有邮件都预测为正常邮件（负类），召回率将显示为0\%，但显然这种模型并无实际应用价值。因此，将召回率与其他评估指标结合使用，可以有效弥补其不足之处。
...
F1分数的主要价值在于其在类别不平衡场景中的有效性。例如，在医疗诊断中，若正类样本（患者）仅占1\%，单纯依赖准确率可能导致模型因全预测为负类而虚高至99\%，而F1分数则能够有效揭示这一缺陷。实验结果显示，当癌症筛查模型的召回率为80\%、精确率为50\%时，其F1分数为61.5\%，这一综合指标比单独关注某一指标更能真实反映模型的性能。F1分数的取值范围为0到1，数值越接近1，表明模型在精确性与全面性上越为均衡。

尽管F1分数具有诸多优点，但其也存在明显的局限性。首先，它完全忽略了真负例（TN）的影响，在负类样本占比极高的场景中可能导致偏差；其次，其默认的等权重设计可能不符合某些特定业务需求，例如在金融风控领域，精确率可能更为重要。

在实际应用中，F1分数在文本分类、欺诈检测等领域表现尤为突出。例如，在电商平台检测虚假评论时，模型A（精确率90\%/召回率45\%）与模型B（精确率60\%/召回率90\%）的F1分数分别为60\%和72\%，后者因其更高的综合性能而成为优选方案。针对多分类问题，可以通过宏平均（Macro-F1）或微平均（Micro-F1）来扩展F1分数的应用，前者平等对待各类别，后者则更关注大类别样本的表现。
...
在实际应用中，Spearman系数具有广泛的适用性。在社会科学领域，它可以用于分析教育水平与收入之间的关系；在医学研究中，能够评估血压与年龄的相关性；在工程领域，则适用于分析半导体芯片性能指标之间的关联。其主要优势在于能够处理定序数据，并且不受异常值或数据分布形态的过度影响。然而，需要注意的是，当样本量较小时，其统计效力可能会降低，并且对完全相同秩次的处理需要特殊方法。
...
根据系数绝对值的大小，相关性的强度可分为五个等级：0.8至1.0为极强相关，0.6至0.8为强相关，0.4至0.6为中等相关，0.2至0.4为弱相关，而0.0至0.2则表示极弱或无相关。值得强调的是，强相关性并不等同于因果关系，可能受到第三方变量或偶然因素的影响。在金融领域，该系数常用于分析证券收益率的关联性；在医学研究中，能够评估临床指标之间的关系；而在工程领域，则用于分析工艺参数与产品性能之间的联系。
